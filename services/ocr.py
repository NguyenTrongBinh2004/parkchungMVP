...
"""
ocr.py — Nhận diện biển số xe
- Nếu easyocr được cài (local / server đủ RAM) → dùng EasyOCR
- Nếu không có easyocr (free tier thiếu RAM) → trả về chuỗi rỗng, không crash
"""
import re
import numpy as np

# Import tùy chọn — không crash nếu thiếu thư viện
try:
    import easyocr
    from PIL import Image, ImageFilter, ImageEnhance
    import io
...
def nhan_dien_bien_so(du_lieu_anh: bytes) -> str:
    ...
    Trả về chuỗi biển số hoặc chuỗi rỗng nếu không nhận diện được.
    """
    if not _OCR_AVAILABLE or DocBienSo is None:
        # EasyOCR chưa được cài (free tier) → trả về rỗng
        # Frontend sẽ hiển thị ô nhập tay
        return ""
    ...
    try:
        ...
        tim_thay = re.search(pattern, van_ban_sach)
        if tim_thay:
            return f"{tim_thay.group(1)}-{tim_thay.group(2)}-{tim_thay.group(3)}"
        return van_ban_sach if van_ban_sach else " | ".join(ket_qua)
    except Exception:
        return ""