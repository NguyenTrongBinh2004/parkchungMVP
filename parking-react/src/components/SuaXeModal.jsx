...
export default function SuaXeModal({ phienId, onClose, onSuccess }) {
  ...
  return (
    <Modal onClose={onClose} title="✏️ Sửa thông tin xe">
      ...
      {!loading && detail && (
        <>
          {/* Chỉ báo lỗi nếu là XE THƯỜNG và đã ra bãi */}
          {!detail.la_ve_thang && !detail.dang_trong_bai && (
            <Alert type="danger">Xe này đã ra bãi, không thể sửa thông tin.</Alert>
          )}

          {/* Cho phép sửa nếu là VÉ THÁNG (bất kể trong bãi hay không) hoặc XE THƯỜNG ĐANG TRONG BÃI */}
          {(detail.la_ve_thang || detail.dang_trong_bai) && (
            <>
              {detail.la_ve_thang && (
